...
export function SearchForm({ defaultKeyword = "", className }: SearchFormProps) {
  ...
  return (
    ...
    >
      <label className="flex w-full items-center gap-2 rounded-full border-2 border-[var(--brand-primary)] bg-[#f5f5f5] px-3 py-2">
        <input
        ...
        />
        <button
          className={`shrink-0 rounded-full bg-[var(--brand-primary)] px-4 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-[var(--brand-deep)] ${isPending ? "opacity-70" : ""}`}
          disabled={isPending}
          type="submit"
        >
          {isPending ? "搜索中" : "搜索"}
