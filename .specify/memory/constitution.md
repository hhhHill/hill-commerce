...
### III. Feature Specs Are Canonical
各 feature spec（`specs/<feature>/`）是当前商城 MVP 的 canonical baseline。每个 feature spec 独立承载其范围内的需求和实现计划。

新建功能时直接在 `specs/` 下创建独立 feature 目录，包含 `spec.md` / `plan.md` / `tasks.md` 三份文件。不再使用单一 umbrella baseline spec。

跨模块约束（如认证、角色、API 前缀约定）在各 feature spec 中各自声明依赖，冲突时以 `.specify/memory/constitution.md` 的 Project Constraints 为准。
...
- 新规范直接落到 `specs/<feature>/` 独立 feature 目录
- 在没有隔离测试库或隔离数据库实例时，禁止运行包含全表清理、全量删除或其他会影响真实开发数据的集成测试；此类测试必须改为只清理测试前缀数据，或迁移到隔离环境后再执行
...
- `specs/*` 下的相关 feature spec 是否需要同步更新
